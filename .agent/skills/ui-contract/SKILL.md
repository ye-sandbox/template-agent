---
name: ui-contract
description: Derives or updates .agent/INTERFACE.md from a backend (screens, actions, bindings, empty/error states, chrome: theme/locale/surface). Use for a first UI spec or after an API change (Desatualizado). Does not write UI code.
---
...
Padronizar o handoff backend → UI: telas, ações, campos obrigatórios, bindings HTTP e NFRs **derivados de evidência**. Chrome (tema, locale, **superfície**) é **acordo humano** na seção 8, não persona inferida da API e não tela derivada de rota. Locale rascunho = `fixed` `en`; superfície rascunho = eixos `pendente`.
...
Se o arquivo já existir, **atualize** (diff de telas/rotas); não apague seções aprovadas sem dizer o que saiu. Status → `Desatualizado` até o humano reaprovar. Locale e superfície já acordados na seção 8 **não** se reabrem, salvo o humano pedir.
...
Não invente tela para “completar o produto”. Sem evidência → seção **Fora de escopo** ou **Adiado**, não tela nova. Chrome (seção 8) é o que **não** precisa de rota: tema sempre; locale começa `fixed` `en`; superfície começa com eixos `pendente`. Locale e superfície só fecham no Passo 5.
...
- Override de superfície **só** se a tela divergir do global (senão omitir; herda §8)

NFRs de domínio só com evidência: rate limit (`429` + `Retry-After`), cooldown, polling vs push, chave de API opcional, timeouts. Superfície **não** é NFR: não cite rota como evidência de audiência.
...
- **Superfície (rascunho):** os cinco eixos `pendente`. Não preencha `operator`/`minimal`/etc. por chute a partir do README.
- Perfil de usuário com tema/locale no schema → binding na tela de conta, não chrome duplicado.

### Passo 5 — Parar e acordar locale e superfície
...
4. **Superfície (obrigatório neste passo):** mostrar a tabela de eixos (audiência, densidade, motion, copy, formato) com valores fechados. Perguntar o default global. Só então preencher a §8. Override por tela: só se o humano apontar um `scr-*` que não herda (ex. home `customer`+`touch` e `scr-ops` `operator`+`desktop`). Não invente override “por simetria”.
5. Marque `Acordo humano: sim` **em locale e em superfície**. Sem os dois o contrato não está aprovado.
6. Se pedirem HTML, Svelte ou app no mesmo turno: recuse. Próximo agente = `ui-prototype` (primeira vez ou delta visual) **depois** deste arquivo aprovado. Stitch: o brief humano deve repetir os eixos acordados **antes** do export. O humano copia este arquivo para o repo de frontend — não deixe duas versões divergirem.
...
| O que mudou | Contrato | `ui-prototype` | `ui-port` |
| :--- | :--- | :--- | :--- |
...
| Superfície global ou override de ficha | Seção 8 / ficha | Sim se o visual aprovado mudar | Não acrescente motion/hover/peça que o proto não tem |
...
- **Chrome** — casca global (tema, locale, política de superfície). Não é tela. Não deriva de inventário de rotas.
- **Superfície** — audiência, densidade, motion, copy, formato (`desktop` \| `touch` \| `kiosk`). Enum fechado. Acordo humano no Passo 5. Override opcional na ficha.
- **Âncora** — `id` / `name` / `data-state` / `data-theme` / `data-audience` / `data-density` / `data-motion` / `data-copy` / `data-format` que o proto e o porte devem copiar (seção 9)
...
- ⚠️ **NÃO** aprove o contrato com locale `pendente` nem com eixo de superfície `pendente`.
- ⚠️ **NÃO** infira audiência/densidade/motion da API ou do nome do repo (isso inventa persona).
- ⚠️ **NÃO** preencha override em toda ficha; vazio = herda a §8.
...
- 💡 **FAÇA:** no Passo 5, fechar os cinco eixos de superfície com o humano (valores da tabela do molde, nada fora da lista).
...
- [ ] Seção 8: superfície — cinco eixos preenchidos (não `pendente`); acordo humano no Passo 5
- [ ] Override de superfície só nas fichas que o humano marcou como divergentes
...
- [ ] Mapa + locale + superfície apresentados para revisão humana
